...
async function readSse(response, { onToken, onEvent }) {
  ...
  async function consumeFrame(frame) {
    ...
    if (event === "token" && typeof payload.token === "string") {
      receivedToken = true;
      try {
        void Promise.resolve(onToken?.(payload.token)).catch(() => {});
      } catch {
        // UI reveal failures must not stop reading the server stream.
      }
...
export class OllamaAgentProvider {
  constructor(options = {}) {
    ...
    this.agentContract = options.agentContract ?? "portfolio-agent-v1";
    this.clientRelease = options.clientRelease ?? "0.0.3";
    this.sessionId = null;
  }

  async generate({ question, pageContext, queryScope, preferCachedFollowUps, clarificationReply, onToken, onEvent, signal }) {
    ...
    const response = await fetch(this.endpoint, {
      ...
      body: JSON.stringify({
        question,
        agentContract: this.agentContract,
        clientRelease: this.clientRelease,
    ...
    return {
      ...result,
      sessionReplaced: Boolean(requestedSessionId && result.sessionId && requestedSessionId !== result.sessionId)
    };
  }
  ...
  async resetSession() {
    ...
    await fetch(this.resetEndpoint, {
      ...
      body: JSON.stringify({ sessionId, agentContract: this.agentContract })
    }).catch(() => {});
  }
}
