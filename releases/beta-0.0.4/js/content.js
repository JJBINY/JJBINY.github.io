import { resolvePublicAssetUrl, runtimeConfig } from "./config.js";
...
function scopeDocumentAssets(html) {
  return html.replace(/\b(src|href)="((?:assets|data)\/[^"]+)"/gu, (_match, attribute, assetPath) => (
    `${attribute}="${resolvePublicAssetUrl(assetPath)}"`
  ));
}
...
export async function loadPortfolioContent() {
  ...
  const documentHtml = await Promise.all(projects.map((project) => (
    fetchResource(resolvePublicAssetUrl(project.documentHtmlPath), "text")
  )));
  ...
  return {
    ...
    projects: projects.map((project, index) => ({
      ...project,
      documentHtml: scopeDocumentAssets(documentHtml[index])
    }))
  };
}
